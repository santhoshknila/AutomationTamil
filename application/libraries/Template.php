...
class Template {
   ...
   {
      if (file_exists(APPPATH .'views/'. $this->group . '/' . $filename) or file_exists(APPPATH .'views/'.$this->group . '/' . $filename . EXT))
      {
   ...
   function initialize($props)
   {
      ...
      if (isset($props['template']) 
         && (file_exists(APPPATH .'views/'. $this->group . '/ ' . $props['template']) or file_exists(APPPATH .'views/'. $this->group . '/' . $props['template'] . EXT)))
      {
      ...
      else 
      {
         ...
         show_error('Either you have not provided a master template or the one provided does not exist in <strong>'. APPPATH .'views/'.$this->group.'</strong>. Remember to include the extension if other than ".php"');
      }
   ...
   function write_view($region, $view, $data = NULL, $overwrite = FALSE)
   {
      ...
      if (count($args) >= 1)
      {
         ...
         { 
            if (file_exists(APPPATH .'views/'. $this->group . '/' . $suggestion . EXT) or file_exists(APPPATH .'views/'. $this->group . '/' . $suggestion))
            {
               // Just change the $view arg so the rest of our method works as normal
               $view =  $this->group . '/' . $suggestion;
               break;
            }
         }
      }
   ...
   function add_js($script, $type = 'import', $defer = FALSE, $footer = false)
   {
      $success = TRUE;
      $js = NULL;
      ...
      {
         $this->js[] = $js;
         if(!$footer) $this->write('_scripts', $js);
         else $this->write('_footer_scripts', $js);
      }
   ...
   function render($region = NULL, $buffer = FALSE, $parse = FALSE)
   {
      ...
      else
      {
         ...
         if ($this->parse_template === TRUE or $parse === TRUE)
         {
            ...
            $output = $this->CI->{$this->parser}->{$this->parser_method}( $this->group . "/" . $this->master, $this->output, TRUE);
         ...
         else
         {
            ...
            $output = $this->CI->load->view( $this->group . "/" . $this->master, $this->output, $buffer);
         }
      }
