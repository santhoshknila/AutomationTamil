...
class Users_model extends CI_Model{
    ...
    public function check_usersdelete($data = array(), $set_session = true){
		
       $user = $this->db->get_where("users",array('emailid' =>trim($data["username"]), 'status' => 0));
    ...
    public function check_usersdeletelogout($data = array(), $set_session = true){
		
		  
       $user = $this->db->get_where("users",array('userid' =>trim($data["users_id"]), 'status' => 0));
	...
	public function getmySubscription($userid){
		$query = $this->db->query("SELECT * FROM userpackages AS userpack WHERE (userpack.packType=1 OR userpack.packType=2) AND userpack.packId != 0 AND status in(1,5) AND userpack.userid = '".(int)$userid."' ORDER BY startDate DESC");
		//echo $sql = $this->db->last_query();
		//exit;
	...
	public function getActiveSubscription($userid){
		$query = $this->db->query("SELECT * FROM userpackages AS userpack WHERE (userpack.packType=1 OR userpack.packType=2) AND userpack.packId != 0 AND status in(1) AND userpack.userid = '".(int)$userid."' ORDER BY startDate DESC");
		//echo $sql = $this->db->last_query();
		//exit;
		return $mySubscripe = $query->result_array();	
	}
	...
	public function deletesubscription($subdata)
		{
					
			$query = $this->db->select('*')->from('userpackages')->where('userPackId',$subdata['userPackId'])->get()->row();
			if($query){
				$query = $this->db->query("UPDATE userpackages SET status=5 WHERE userPackId ='".$subdata['userPackId']."'");	
              return true;
		 	 }else{
				return false;
			}
