...
class Products_model extends CI_Model{
	...
	public function save_update_products($data = array()){	
		...
		if(!empty($data['pro']['productId'])){
			...
			if($affected_rows == 1){
				...
				// $imgremove = explode('-', $data['proImgremove']);
				
				// 	foreach ($imgremove as $key => $img) {
				// 		$this->db->where('proimageId', $img);
				// 		$this->db->where('productId', $data['pro']['productId']);
	...
	public function getallAds($data = array()){
		...
		if(!empty($data['range']) && $data['range'] == "lowtohigh"){
		 ...
		 $query = $this->db->query("SELECT * FROM products AS pro WHERE pro.proType=2 AND pro.status=1 AND pro.reportAbuse=1 AND pro.procatId!=0 AND endDate >= CURDATE() $sql");
	...
	public function getallProducts($data = array()){		
		...
		$sql .= " AND NOT pro.packageUnique in(select packageUnique from userpackages Where packId>0 AND DATE(endDate) < DATE(now()))";
		//print_r($sql);exit;
		...
		//print_r("SELECT * FROM products AS pro WHERE pro.status=1 AND pro.reportAbuse=1 AND pro.procatId!=0 ".$sql." limit ".(int)$data['limit']." offset ".(int)$data['start']); exit;
	...
	public function getProductstotal($data = array()){
		...
		//print_r("SELECT * FROM products AS pro WHERE pro.status IN(1) AND pro.reportAbuse=1 AND pro.procatId!=0 " .$sql); exit;
		$query = $this->db->query("SELECT * FROM products AS pro WHERE pro.status IN(1) AND pro.reportAbuse=1 AND pro.procatId!=0 ".$sql);
		
		return $adsResultCnt = $query->result_array();	
	}
	
	public function getProduct($data = array()){
		$query = $this->db->query("SELECT * FROM products AS pro WHERE   pro.productId='".(int)$data['productId']."'");
		//echo $sql = $this->db->last_query();
		//exit;
		return $adsResult = $query->result_array();	
	}
