...
class Products extends CI_Controller {
	...
	public function index(){
		...
		if(checkForUserSession($userauth) == 1){
			...
			$mySelectpack = $this->users_model->getActiveSubscription($userauth['users_id']);
		...
		if(checkForUserSession($userauth) == 1){
			...
			if(isset($post) && !empty($post)){
				...
				*/
				...
				/* 24/7/2019 alter for product update start */
				...
				/* 24/7/2019 alter for product update end */
				...
				*/
				...
				if(count($myProducts) >= 3 && (!empty($post['adsId']) || !empty($post['packageId']))){
					...
					{
						$response['messages'] = 'Products has been updated successfully';
					}else{
	...
	public function getproductInfo($proId){
		...
		if(checkForUserSession($userauth) == 1){
			if(isset($proId)){
				...
				$getProducts = $this->products_model->getProduct($proID_data);			
				$response['products'] = array();
				if (!empty($getProducts)) {
					foreach($getProducts as $product){
						...
						$locationmain =  $this->general_model->getCountries($product['prolocId']);						
						...
						$locationsub = $this->general_model->getProvinceName($product['prosublocId']);
						...
						$response['products'] = array(
							...
							'proCountryId' => $product['proCountryId'],
							'propackageId' => $product['packageId'],
							'propackageUnique' => $product['packageUnique'],
							'proadsID' => $product['adsId'],				
							'proType' => $product['proType'],				
							'prostatus' => $product['status'],	
							'proDisplaytype' => $product['proDisplaytype'],	
