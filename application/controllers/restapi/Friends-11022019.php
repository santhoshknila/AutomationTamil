				...
				if(!empty($feiendSts)){				 
					...
					$notify_data = array(
						array(
							...
							'notifyType'	=> 'Friend',
							'notifyReason'	=> 'You and <b>'.$userdetailsSent->firstName.' '.$userdetailsSent->surName.'<b> become friend',
							'createdBy'		=> $userauth['users_id'],							
							'updatedBy'		=> $userauth['users_id'],
						),
						array(
							...
							'toUserId'      => $userauth['users_id'],
							'notifyType'	=> 'Friend',
							'notifyReason'	=> 'You and <b>'.$userdetails->firstName.' '.$userdetails->surName.'<b> become friend',
							'createdBy'		=> $userauth['users_id'],							
							'updatedBy'		=> $userauth['users_id'],
						)
					);
		...
		if(checkForUserSession($userauth) == 1){	
			...
			if(!empty($friRequest)){
				foreach($friRequest as $userList){	
					...
					$mutualFriend = $this->friends_model->getMutualfriendsabout($userauth['users_id'], $post['friendId']);
					$listUserarray[] = array(
						'friendID' => $userList['friendId'],
						'userId' => $userID,
						'userName' => $firstName,
